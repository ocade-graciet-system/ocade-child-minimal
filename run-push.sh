...
npm test && git fetch origin master && git checkout FETCH_HEAD -- style.css && sleep 5 && git add $add && git commit $commit -m "$message" && git pull origin $branch && git push origin $branch --force
